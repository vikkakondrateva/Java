...
public class lab1java extends javax.swing.JFrame {
    ...
    private List<ClientConnection> clientSocket = new ArrayList<>();
    private int countClient = 2;
    ...
    public lab1java() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            for (int i = 0; i < countClient; i++){
                ...
                clientSocket.add(new ClientConnection(newClient));
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        for (int i = 0; i < countClient; i++){
            if(i == 0){
                try{
                    RecIntegral rec = new RecIntegral(s_lowerLimit, Double.toString(half), s_step, Double.toString(res));
                    clientSocket.get(i).getOut().writeObject(rec); // Берем i-го клиента, получаем его поток вывода (ObjectOutputStream) и отправляем ему объект rec
                }
                catch (InvalidNumberException e){
                    JOptionPane.showMessageDialog(lab1java.this, "Ошибка: " + e.getMessage(), "Некорректные данные", JOptionPane.WARNING_MESSAGE);
                }
                ...
                try{
                    RecIntegral rec = new RecIntegral(Double.toString(half), s_upperLimit, s_step, Double.toString(res));
                    clientSocket.get(i).getOut().writeObject(rec); 
                }
                catch (InvalidNumberException e){
                    JOptionPane.showMessageDialog(lab1java.this, "Ошибка: " + e.getMessage(), "Некорректные данные", JOptionPane.WARNING_MESSAGE);
                }
        ...
        for (ClientConnection client : clientSocket) {
            try{
                res += client.getIn().readDouble();
            }
            catch(IOException e){
